...
public enum DecisionType { // Outcome type of decision
	BLOCK_SOURCE {
        @Override
        public int getOrder() {
            return 5;
...
//	AUTO_SUPPRESS, //  feedback to detection to skip the specific type of detection.
    static public int getOrder(String sValue) {
    	if (sValue == null || sValue.isEmpty())
    		return -1;
    	try {
    		DecisionType t = DecisionType.valueOf(sValue);
    		return t.getOrder();
    	} catch (Exception ex) {
    		return -1;
    	}
	...
	static public com.essence.model.DecisionType textToValue(String s) {
		if (s==null || s.isEmpty())
			return null;
		try {
			return com.essence.model.DecisionType.valueOf(s);
		} catch (Exception ex) {
			return null;
		}
	}

...
from AnnotatedNetworkGraphJSONGenerator import *
...
@app.before_request
def csrf_protect():
	if request.user_agent.browser == "foo" or request.user_agent.browser == "bar":
...
def load_user(username):
	...
	fd = open("./data/User/" + username + ".json")
	user = User(json.load(fd))
	fd.close()
	return user
...
def login_page():
	...
	return render_template("AnnotatedNetworkGraphLogin.html", next=nextUrl)
...
@app.route("/feederData/<owner>/<feederName>/",	 methods=["GET"]) 
@app.route("/feederData/<owner>/<feederName>/<modelFeeder>", methods=["GET"])
@flask_login.login_required
def feederData(owner, feederName, modelFeeder=False):
		since = map(request.form.get, ["since"])
		gData = getGraph()
		return gData

#This is done because essence-services does not implement CORS
@app.route("/endpointtypes", methods=["GET"])
def endpointTypes():
	msversion = request.args.get("version")
	return getEndpointTypes(msversion)

@app.route("/alerts/<srcName>", methods=["GET"])
def alerts(srcName):
	return getAlerts(srcName)

#This is done because essence-services does not implement CORS
@app.route("/detectionruletypes")
def detectionRuleTypes():
	return getDetectionRuleTypes()

@app.route("/rulesfornode", methods=["GET"])
@flask_login.login_required
def rulesForNode():
	ip = request.args.get("ip")
	return getDetectionRulesForNode(ip)

@app.route("/saveannotations", methods=["POST"])
@flask_login.login_required
def saveAnnotations():
	forbiddenKeys = ["node_ip_address",
					 "edittextarea",
	...
	for key in request.form:
		if key.endswith("value_pair") or key in forbiddenKeys:
			continue
		if str(key + "value_pair") not in request.form:
			return "fail", key, " does not have a key"
		keysToSave[request.form[key]] = request.form[str(str(key) + "value_pair")]
	if 'edittextarea' in request.form:
		keysToSave['edittextarea'] = request.form['edittextarea']
	ip = request.form["node_ip_address"]
	print ip, keysToSave
	return saveAnnotationsToDatabase(ip, keysToSave)

@app.route("/savetooltipdisplay", methods=["POST"])
@flask_login.login_required
def savetooltipdisplay():
	tooltipdisplay = int(request.form.get("tooltipdisplay"))
	return saveSetting("tooltipdisplay", tooltipdisplay)
	
@app.route("/gettooltipdisplay", methods=["GET"])
@flask_login.login_required
def gettooltipdisplay():
	return getSetting("tooltipdisplay")
...
@flask_login.login_required
def root():
		return render_template("gridEdit.html", feederName="andre", ref=request.referrer,
				is_admin=User.cu()=="admin", modelFeeder=False, public=False,
				currUser = User.cu(), owner = "admin")
...
if __name__ == "__main__":
	URL = "http://10.21.1.27:5000"
	template_files = ["templates/"+ x  for x in os.listdir("templates")]
	model_files = ["models/" + x for x in os.listdir("models")]
	app.run(host='0.0.0.0', port=5000, debug=True, extra_files=template_files + model_files)

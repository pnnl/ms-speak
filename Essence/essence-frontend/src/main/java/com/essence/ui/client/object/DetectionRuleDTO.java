...
public class DetectionRuleDTO implements Serializable {
	@Id @GeneratedValue
	@Column(name = "rule_id")
	private int	   id;
	...
	transient List<ValueOutOfBoundDetailDTO> voobDetails = null;
	...
	public void setRuleType(DetectionRuleTypeDTO ruleType) {
	...
	public List<ValueOutOfBoundDetailDTO> getVoobDetails() {
		return voobDetails;
	}
	public void setVoobDetails(List<ValueOutOfBoundDetailDTO> voobDetails) {
	...
	public int getNonHeaderIndexOfVooBDetails() {
		...
		if (voobDetails != null) {
    		for (msgIdx=0; msgIdx<voobDetails.size(); msgIdx++) {
    			if (!voobDetails.get(msgIdx).getEndpointCode().equals(XpathDTO.HEADER_SERVICE_CD))
    				break;
    		}
	...
	public void print() {
		...
		if (voobDetails != null && !voobDetails.isEmpty())
			for (ValueOutOfBoundDetailDTO d : voobDetails)
				sb.append("\t" + d.getDisplayDetails());
		System.out.println(sb.toString());
	}

	public DetectionRuleDTO(int id, int organizationId, String ruleType, String srcEndpointType, String dstEndpointType, String version, String actionType, String srcIPAddress, String dstIPAddress, Integer numberOfPacketsForDoS, Long timeWindowInSeconds, Integer priority, String voobTitle, List<ValueOutOfBoundDetailDTO> voobDetails, String policyDescription) {
		this.id = id;
		this.organizationId = organizationId;
		this.ruleType = ruleType;
		this.srcEndpointType = srcEndpointType;
